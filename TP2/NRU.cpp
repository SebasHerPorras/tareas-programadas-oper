...
int NRU(const std::vector<int>& references, int frames, const std::vector<int>& initial_state) {
  ...
  for (int ref : references) {
    ...
    if (page_table.find(page) != page_table.end()) {
      std::cout << YELLOW << "[NRU] Página " << page << " ya está en memoria" << RESET << std::endl;
      auto* entry = page_table[page];
      entry->reference_bit = true;
      if (is_modified) entry->modified_bit = true;
    } else {
      page_faults++;
      std::cout << RED << "[NRU] Fallo de página - Cargando: " << page;
  ...
  std::cout << RED << "[NRU] Total de fallos de página: " << page_faults << RESET << std::endl;
  return page_faults;
}
